...
function success({ products }: Props) {
  ...
  const {data : session} = useSession()
	...
	return (
      ...
      <Link href="/">
          <div className="relative ml-4 h-24 w-12 cursor-pointer transition hidden lg:inline-flex">
            <Image
              src="https://rb.gy/vsvv2o"
              layout="fill"
              objectFit="contain"
        ...
        <div className="flex h-11 w-11 items-center justify-center rouunded-full border-2 border-black">
         <CheckIcon className="h-8 w-8"/>
        </div>
        <div>
        <p className="text-sm text-gray-600">Order #{session_id?.slice(-5)}</p>
        <h4 className="text-lg ">Thank you{" "} 
...
<p>We've accepted your order, and we're getting it ready. come back to this page on your shipment status</p>
</div>
...
<p>Order updates</p>
<p className="text-sm text-gray-600">You'll get shipping and delivery updates by email or text.</p>
</div>
<div className="mx-4 flex flex-col items-center justify-between text-sm lg:ml-14 lg:flex-row">
<p className="hidden lg:inline">Need help? Contact Us</p>
{mounted && (
<Button title="Continue Shopping" onClick={() => router.push("/")} width={isTabletOrMobile ? "w-full" : undefined} padding="py-4" />
	)}
</div>
      </section>
      {mounted && (
